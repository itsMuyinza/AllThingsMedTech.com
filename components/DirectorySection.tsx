...
const DirectorySection: React.FC<DirectorySectionProps> = ({ onNavigate }) => {
    ...
    <section id="directory" className="py-20 bg-white relative">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-20">
          {categories.map((cat) => (
            <div 
                key={cat.name} 
                onClick={() => {
                    console.log("Navigating to:", cat.filterName);
                    if (onNavigate) onNavigate('directory', { category: cat.filterName });
                }}
        ...
        <div className="relative">
          ...
          <div 
            ref={scrollRef}
            className="flex gap-6 overflow-x-auto pb-8 snap-x scrollbar-hide no-scrollbar"
              ...
              <div key={idx} className="snap-start min-w-[280px] h-[360px] bg-bone rounded-2xl p-6 border border-med-teal/10 flex flex-col justify-between group hover:border-retro-orange/50 transition-colors shadow-sm relative overflow-hidden">
                ...
                <div className="z-10">
                    <button 
                        ...
                        className="w-full py-3 rounded-lg border border-ink/10 font-tech text-sm font-bold text-ink hover:bg-med-teal hover:text-white hover:border-med-teal transition-all"
                    >
                        View Profile
                    </button>
                </div>
