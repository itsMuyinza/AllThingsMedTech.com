...
const Header: React.FC<HeaderProps> = ({ onNavigate, currentView, isLoggedIn = false }) => {
  ...
  const handleSearchSubmit = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
        onNavigate('directory', { query: searchQuery }); // Navigate to directory filtering
        setIsSearchOpen(false);
    }
  };

  const navLinks = [
    { name: 'Directory', hasMega: true, action: () => onNavigate('directory') },
    { name: 'News', hasMega: false, action: () => onNavigate('news') },
    { name: 'Podcasts', hasMega: false, action: () => onNavigate('podcasts') },
    { name: 'Resources', hasMega: false, action: () => onNavigate('whitepapers') },
    { name: 'Events', hasMega: false, action: () => onNavigate('events') },
    { name: 'About', hasMega: false, action: () => onNavigate('about') },
  ];
                    ...
                    <div className="space-y-3">
                      ...
                      <ul className="space-y-2 text-sm text-ink/70">
                        {['Adhesives', 'Cleanroom', 'Components', 'Materials', 'Packaging'].map(i => (
                          <li key={i}><a href="#" onClick={(e) => {e.preventDefault(); onNavigate('directory', { category: i });}} className="hover:text-retro-orange transition-colors block py-1 hover:translate-x-1 duration-200">{i}</a></li>
                        ))}
                      </ul>
                    </div>
                    <div className="space-y-3">
        ...
        <div className="flex items-center space-x-4">
          ...
          {isLoggedIn ? (
              <Button 
                variant="primary" 
                size="sm" 
                className="hidden md:inline-flex shadow-retro-orange/20"
      ...
      {isMobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-bone/95 backdrop-blur-xl border-b border-med-teal/10 shadow-xl p-6 md:hidden flex flex-col space-y-4 animate-[slideDown_0.3s_ease-out]">
           ...
           <div className="pt-4 border-t border-med-teal/10 flex flex-col gap-3">
              <Button variant="outline" className="w-full" onClick={() => onNavigate(isLoggedIn ? 'dashboard' : 'login')}>{isLoggedIn ? 'Dashboard' : 'Log In'}</Button>
              {!isLoggedIn && <Button variant="primary" className="w-full" onClick={() => onNavigate('join')}>Sign Up Free</Button>}
           </div>
        </div>
      )}
    </header>
