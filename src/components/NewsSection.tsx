...
const NewsSection: React.FC = () => {
  const articles = [
    { id: 1, category: 'Regulatory', title: 'FDA Updates Guidance on AI-Enabled Medical Devices', summary: 'New framework aims to streamline the approval process for machine learning algorithms in software as a medical device (SaMD). The industry is bracing for more rigorous validation requirements while welcoming the potential for faster breakthrough designations.', date: 'Oct 14', image: 'https://picsum.photos/seed/medtech1/800/600', featured: true },
    { id: 2, category: 'Innovation', title: 'Robotics Startup "SurgiBot" Raises Series B', summary: 'The funding will accelerate the development of their haptic feedback surgical system.', date: 'Oct 12', image: 'https://picsum.photos/seed/medtech2/400/300', featured: false },
    { id: 3, category: 'Supply Chain', title: 'Solving the Titanium Shortage in Orthopedics', summary: 'Manufacturers are turning to additive manufacturing to reduce waste and cost.', date: 'Oct 10', image: 'https://picsum.photos/seed/medtech3/400/300', featured: false },
    { id: 4, category: 'M&A', title: 'Boston Scientific Acquires Relievant Medsystems', summary: 'A strategic move to expand their chronic pain management portfolio.', date: 'Oct 09', image: 'https://picsum.photos/seed/medtech4/400/300', featured: false },
    { id: 5, category: 'Digital Health', title: 'Wearable Biosensors Market Projected to Hit $50B', summary: 'Consumer demand for continuous health monitoring is driving massive growth.', date: 'Oct 08', image: 'https://picsum.photos/seed/medtech5/400/300', featured: false },
  ];

  const latestFeed = [
    { id: 10, category: 'Deep Dive', title: '4 medtech topics to watch in 2026', summary: 'From insurance coverage questions to M&A and tariffs, here are the top storylines to watch in the medical device space.', image: 'https://picsum.photos/seed/medlatest1/800/500', date: 'Oct 15' },
    { id: 11, category: 'Innovation', title: 'Intuitive Surgical procedures jump 20% in latest quarter', summary: 'The robotic surgery giant continues its dominance with strong placement of its latest generation platforms.', image: 'https://picsum.photos/seed/medlatest2/800/500', date: 'Oct 14' },
    { id: 12, category: 'Cardiovascular', title: 'Abbott expands heart valve portfolio with CE Mark', summary: 'New minimally invasive options are becoming available for patients with tricuspid regurgitation.', image: 'https://picsum.photos/seed/medlatest3/800/500', date: 'Oct 13' },
    { id: 13, category: 'FDA', title: 'Baxter gets 510(k) for new infusion pump platform', summary: 'The updated system focuses on cybersecurity enhancements and nursing workflow optimization.', image: 'https://picsum.photos/seed/medlatest4/800/500', date: 'Oct 12' },
    { id: 14, category: 'Earnings', title: 'Stryker raises full-year guidance on strong ortho demand', summary: 'Recovery in elective procedure volumes drives record revenue for the medical technology leader.', image: 'https://picsum.photos/seed/medlatest5/800/500', date: 'Oct 11' },
  ];
    ...
    <div className="space-y-20">
      ...
      <section className="py-16 px-6 max-w-7xl mx-auto relative z-20 bg-bone/50 backdrop-blur-sm rounded-3xl shadow-sm border border-white/50">
        <div className="flex items-end justify-between mb-12 border-b-2 border-ink/10 pb-4">
          ...
          <Link href="/news" className="hidden md:flex items-center gap-2 font-tech font-bold text-sm text-ink hover:text-retro-orange transition-colors group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-x-10 gap-y-8">
          ...
          <Link href="/news" className="lg:col-span-8 group cursor-pointer block">
            <div className="relative overflow-hidden rounded-2xl border border-ink/5 shadow-lg h-[460px]">
              ...
              <img src={featuredArticle.image} alt={featuredArticle.title} className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700 ease-out filter grayscale hover:grayscale-0" />
            ...
            {sideArticles.map((article, idx) => (
              ...
              >
                <div className="w-24 h-20 shrink-0 overflow-hidden rounded-xl border border-ink/10 relative">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={article.image} alt={article.title} className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-500 filter grayscale hover:grayscale-0" />
                </div>
          ...
          <Link href="/news" className="lg:col-span-12 group cursor-pointer mt-4 block">
            <div className="border-l-4 border-retro-orange pl-8 py-2">
              <div className="flex items-center gap-3 text-xs text-ink/50 font-tech mb-3">
                <Clock className="w-3 h-3" /> {featuredArticle.date} &bull; Featured Headlines
              </div>
      ...
      <section className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div className="lg:col-span-9 space-y-16">
          ...
          <div className="space-y-12">
            {latestFeed.map((story, idx) => (
              <React.Fragment key={story.id}>
                <Link href="/news" className="flex flex-col md:flex-row gap-8 group cursor-pointer">
                  <div className="w-full md:w-64 h-44 overflow-hidden rounded-lg shrink-0 border border-ink/5">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={story.image} alt={story.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 grayscale group-hover:grayscale-0" />
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-tech text-ink/30 uppercase">
                      <Clock className="w-3 h-3" /> {story.date} &bull; 3 min read
                    </div>
                  </div>
                </Link>
          ...
          <div className="mt-16 flex justify-center">
            <Link href="/news">
              <Button variant="outline" size="lg">View All Stories</Button>
            </Link>
          </div>
        </div>
